...
const CalendarDisplay = () => {
  const [newEvent, setNewEvent] = useState({title:"", detail:"",start:"", end:""});
  ...
  const onClickBox=(pEvent)=> {
    ...
    // alert(
      <div>
        ...
        />
        <input type="text" placeholder="Add Detail" style={{width:"20%", marginRight:"10px" }}
          value={newEvent.detail} onChange={(e)=> setNewEvent({...newEvent, detail: e.target.value})}
        />
  ...
  return (
    <div style={{paddingTop:"80px",}}>
      ...
      <div style={{paddingLeft:"50px"}}>
        <input type="text" placeholder="Add Title" style={{width:"20%", marginRight:"10px", border:'none', height:'50px', fontSize:'25px' }}
          value={newEvent.title} onChange={(e)=> setNewEvent({...newEvent, title: e.target.value})}
        />
        <input type="text" placeholder="Add Detail" style={{width:"20%", marginRight:"10px", border:'none', height:'50px', fontSize:'25px' }}
          value={newEvent.detail} onChange={(e)=> setNewEvent({...newEvent, detail: e.target.value})}
        />
        <DatePicker placeholderText="Start Date" style={{marginRight:"10px", height:'500px', fontSize:'25px'}} 
          selected={newEvent.start} onChange={(start)=> setNewEvent({...newEvent, start})}
          /> 
          {/* <DatePicker placeholderText="End Date" 
          selected={newEvent.end} onChange={(end)=> setNewEvent({...newEvent, end})}
